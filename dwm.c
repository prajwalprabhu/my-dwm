...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, issticky,ismax,wasfloating;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
static void baroff(const Arg *arg);
...
manage(Window w, XWindowAttributes *wa)
{
	...
	updatesizehints(c);
	updatewmhints(c);
	//uncomment to make floatinf windows at the center
...
void
baroff(const Arg *arg)
{
	selmon->showbar=selmon->pertag->showbars[selmon->pertag->curtag]= False;
	updatebarpos(selmon);
	XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by, selmon->ww, bh);
	arrange(selmon);
}
